...
class MovingMNISTDigits(Dataset):
    def __init__(self, root, split='train', train_ratio=0.8, transform=None):
        ...
        # Load the dataset
        self.videos = np.load(f'downstream_mnist_videos.npy')
        self.labels = np.load(f'downstream_mnist_digit_labels.npy')

        total_samples = self.videos.shape[0]
        train_end = int(total_samples * self.train_ratio)

        if self.split == 'train':
            self.videos = self.videos[:train_end]
        ...
        print(f"{self.split} dataset size: {len(self.labels)}")
...
class MovingMNISTMotions(Dataset):
    def __init__(self, root, split='train', train_ratio=0.8, transform=None):
        self.root = root
        self.split = split
        self.train_ratio = train_ratio
        ...
        # Load the dataset
        self.videos = np.load(f'downstream_mnist_videos.npy')
        self.labels = np.load(f'downstream_mnist_motion_labels.npy')
