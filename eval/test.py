...
class ConsistentRandomRotation:
    ...
    def __call__(self, img):
        if self.rotation is None:
            self.rotation = transforms.RandomRotation.get_params([-self.degrees, self.degrees])
        
        if isinstance(img, list):
            return [transforms.functional.rotate(i, self.rotation, fill=0) for i in img]
        else:
            return transforms.functional.rotate(img, self.rotation, fill=0)

    def randomize(self):
        self.rotation = None

consistent_rotation = ConsistentRandomRotation(degrees=270)
...
def main():
    ...
    elif args.dataset == 'MovingMNISTDigits': args.num_class = 100 
    elif args.dataset == 'MovingMNISTMotions': args.num_class = 6
    ...
    elif args.dataset in ['MovingMNISTDigits', 'MovingMNISTMotions']:
        if args.img_dim == 128:
            lr_lambda = lambda ep: MultiStepLR_Restart_Multiplier(ep, gamma=0.1, step=[300,400,500], repeat=1)
        else:
            lr_lambda = lambda ep: MultiStepLR_Restart_Multiplier(ep, gamma=0.1, step=[60, 80, 100], repeat=1)
    else:
        # Default lr_lambda for any other dataset
        lr_lambda = lambda ep: MultiStepLR_Restart_Multiplier(ep, gamma=0.1, step=[60, 80, 100], repeat=1)
...
def get_data(transform, mode='train'):
    ...
    if args.dataset == 'ucf101':
        ...
        dataset = MovingMNISTDigits(root='',
